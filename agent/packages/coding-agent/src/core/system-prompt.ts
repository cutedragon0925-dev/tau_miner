...
function listSiblingFiles(cwd: string, topFile: string): string[] {
	try {
		const dir = topFile.includes("/") ? topFile.substring(0, topFile.lastIndexOf("/")) : ".";
		const fullDir = resolve(cwd, dir);
		if (!existsSync(fullDir)) return [];
		const entries = readdirSync(fullDir, { withFileTypes: true });
		const siblings = entries
			.filter(e => e.isFile() && e.name !== basename(topFile))
...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		const excludePipe = "grep -v node_modules | grep -v '/\\.git/' | grep -v '/dist/' | grep -v '/build/' | grep -v '/out/' | grep -v '/\\.next/' | grep -v '/target/'";

		// Phase 1: one combined grep call with all keywords to get candidate files.
		// This is faster than N sequential greps for large codebases.
		const allPatterns = filtered.map(kw => `-e "${shellEscape(kw)}"`).join(" ");
		let candidateFiles: string[] = [];
		try {
			const combined = execSync(
				`grep -rlF ${allPatterns} ${includeGlobs} . 2>/dev/null | ${excludePipe} | head -20`,
				{ cwd, timeout: 8000, encoding: "utf-8", maxBuffer: 4 * 1024 * 1024 },
			).trim();
			if (combined) {
				candidateFiles = combined.split("\n").map(l => l.trim().replace(/^\.\//, "")).filter(Boolean);
			}
		} catch { }
			...
			try {
				...
				if (nameResult) {
					for (const line of nameResult.split("\n")) {
						...
						filenameHits.get(file)!.add(kw + " (filename)");
		...
		if (topFile) {
			...
			// Sibling listing: surface adjacent files in the same directory so the model
			// knows what else lives there without spending a tool call on ls.
			const siblings = listSiblingFiles(cwd, topFile);
			if (siblings.length > 0) {
				sections.push(`\nSIBLING FILES in same directory as ${topFile}:`);
				for (const s of siblings) sections.push(`- ${s}`);
				sections.push("Check siblings for wiring/type/config changes required by the task.");
			}
		}
